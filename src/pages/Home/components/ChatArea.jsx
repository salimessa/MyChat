...
import * as messageService from "../../../utilities/message-service";
import { SetAllChats } from "../../../redux/userSlice";
import moment from "moment";
import * as chatService from "../../../utilities/chats-service";
...
function ChatArea({ socket }) {
  ...
  const getMessages = async () => {
    try {
      const response = await messageService.getAllMessages(selectedChat._id);
      if (response.success) {
  ...
  const clearUnreadMessages = async () => {
    try {
      ...
      const response = await chatService.clearUnreadMessages(selectedChat._id);
      if (response.success) {
  ...
  useEffect(() => {
    // always scrol to bottom for messages id
  ...
  const sendNewMessage = async () => {
    try {
      ...
      // send message to server to store in the db
      const response = await messageService.createNewMessage(message);
      if (response.success) {
        setNewMessage("");
    ...
    <div className="bg-white h-[82vh] border rounded-2xl w-full flex flex-col justify-between p-5">
      <div>
        <div className="flex gap-5 items-center mb-2">
          ...
          {!receipentUser?.profilePic && (
            <div className="bg-gray-500  rounded-full h-10 w-10 flex items-center justify-center relative">
              <h1 className="uppercase text-xl text-white font-semibold">
            ...
            messages.map((message, index) => {
              ...
              return (
                <div
                  ...
                  key={index}
                >
                  {/* <h1>{getDateInRegualarFormat(message.createdAt)}</h1> */}
