...
function UsersList({ searchKey, socket, onlineUsers }) {
  ...
  const getData = () => {
    // if search key is empty, return all chats else return filtered chats and users
    if (searchKey === "") {
      return allChats;
    }
  ...
  const getIsSelectedOrNot = (userObj) => {
    if (selectedChat) {
    ...
    if (!chat || !chat.lastMessage) {
      ...
      return (
        <div className="flex justify-between w-72">
          ...
          <h1 className="text-gray-500 tx-sm">
      ...
      {getData().map((chatObjorUserObj) => {
        ...
        return (
          <div
            ...
            ${getIsSelectedOrNot(userObj) && "border-primary bottom-2"}`}
            key={userObj._id}
          ...
          >
            ...
            <div onClick={() => createNewChat(userObj._id)}>
              {!allChats.find((chat) =>
                ...
                <button className="border-primary text-primary border bg-white p-1 rounded">
                  Create Chat
                </button>
              )}
            </div>
